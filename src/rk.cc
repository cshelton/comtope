...
double expmt(matrix &a, const matrix &Q, double t, double eps, double h) {
	double ret = 0.0;
	if (t<=0.0) return ret;
	...
	a = matrix(p,p,0.0);
	for(int i=0;i<p;i++) a[i][i] = 1.0;
	matrix ka1(p,p),ka2(p,p),ka3(p,p),ka4(p,p),ka5(p,p),ka6(p,p), temp(p,p), erra(p,p);
	...
	while(currt<t) {
		...
		for(;;) { // find stepsize
			...
			erra = dc1*ka1+dc3*ka3+dc4*ka4+dc5*ka5+dc6*ka6;
			*/

			ka1 = a*Q;//Q.leftmult(a,ka1);
			ka1 *= h;
			//ka1 = (a*Q)*h;
...
double vexpmt(vectr &a, const spmatrix &spQ, double t, double eps, double h, 
		vector<double> *timesteps) {
//double vexpmt(vectr &a, const matrix &Q, double t, double eps, double h) {
	double ret = 0.0;
	if (t<=0.0) return ret;
...
double expmtv(vectr &b, const spmatrix &spQ, double t, double eps, double h) {
    double ret = 0.0;
	if (t<=0.0) return ret;
...
double vexpmt2(vectr &a, const matrix &Q, double t) {
	...
	if (det>=0.0) {
		...
		ch = (1.0 + ex1)/2.0;
		sh = (1.0 - ex1)/2.0/det;
		if (std::isnan(sh)) sh = 0.5*exp(-det*t/2.0);
