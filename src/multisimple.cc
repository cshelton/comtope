...
double MultiLogSimple::Normalize() {
	...
	int n = logtheta.getm();
	for(int i=0;i<n;i++)
		if (std::isfinite(logtheta[i])) sum += exp(logtheta[i]-lmax);
	lmax -= log(sum);
	for(int i=0;i<n;i++)
		if (std::isfinite(logtheta[i])) logtheta[i] += lmax;
	return exp(-lmax);
}
...
int MultiLogSimple::Sample(Random &rand) const {
	...
	double sum = 0.0;
	for(int i=0;i<n;i++)
		if (std::isfinite(logtheta[i])) addw[i] = sum;
...
void MultiLogSimple::MakeUniform() {
	...
	//if (std::isfinite(logtheta[i])) logtheta[i] = 0.0;
}
