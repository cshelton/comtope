...
ExpMCache::ExpMCache(const matrix &Q, double maxt, double eps, bool negtoo)
			//: expm(Q.getm())
			{
	hasneg = negtoo;
	int m = Q.getm();
 ...
 */
	...
	if (negtoo) {
		matrix eyeneg(m,m,0.0); // eye above was overwritten
		for(int i=0;i<m;i++) eyeneg[i][i] = 1.0;
		ctbn::vexpmt(eyeneg,-Q,maxt,eps,-1.0,negexpm);
	}
}

class vecmatmult {
public:
	vecmatmult(vectr &aa) : a(aa), a0(aa) { a *= 0.0; first = true; }
	vectr &a;
	vectr a0;
	bool first;

	inline void operator()(const matrix &m, double wt) {
		if (first) {
			a.noalias() = (a0*m)*wt;
			first = false;
		} else a += (a0*m)*wt;
...
/* version 2:
 */
	if (hasneg && t<0) a = a*negexpm.GetVal(-t);
	else {
		// below 2 lines are slower -- copying matrix is better
		//vecmatmult vmm(a);
		//expm.ValOp(t,vmm);
		a = a*expm.GetVal(t);
	}
