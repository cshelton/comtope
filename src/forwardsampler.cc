...
ForwardSampler::ForwardSampler(const Process *pr,
...
ForwardSampler *ForwardSampler::Clone() const {
	return new ForwardSampler(*this);
}

void ForwardSampler::SampleTrajectories(vector<Trajectory> &tr, 
...
void ForwardSampler::MakeCache(const Instantiation &inst) {
	nodeindex.clear();
	nodeval.clear();
...
double ForwardSampler::SampleDyn(Trajectory &traj, Random &rand) {
...
//cout << "--------------------------------------------" << endl;
		...
		if (nextid!=-1) {
			...
			int newval = SampleTransition(curri, nextid, nodeindex[nextid], 
					nodeval[nextid], nextt, weight, rand);
			if (oldval!=newval) {
...
double ForwardSampler::SampleSingleTrajectory(Trajectory &traj, bool logscale, Random &rand) {
	double currt = begintime;
