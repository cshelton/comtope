...
void MeanFieldInf::Initialize() {
	...
	for(unsigned int ii=0; ii<varL.size(); ii++) {
		...
		AddVar(curVarId);
		transTimes.insert(pair<int,vector<double> >(
			curVarId,times));
...
double MeanFieldInf::CalcPointCompEnergy(int varid, double t) {
	...
	double ret = std::isnan(res) ? 0 : res;
	assert(!std::isnan(ret) && !isinfinite(ret));
	return ret;
}
...
void MeanFieldInf::AddVar(int varid) {
	mus.insert(pair<int,ContFunction<vectr> >(
			varid, ContFunction<vectr>()));

	gammas.insert(pair<int,ContFunction<matrix> >(
			varid, ContFunction<matrix>()));

	hiddenIntervals.insert(pair<int,vector<double_pair> >(
